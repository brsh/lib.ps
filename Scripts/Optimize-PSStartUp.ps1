...
.DESCRIPTION
    ...
    P.S. This is the first script created using my New-PSScript.ps1 script (ooh - lots of 'script' in that sentence). And I deleted most of the skeleton as unnecessary for this script. How's that for handy!
...
if ( ([System.Environment]::OSVersion.Version.Major -gt 5) -and (
	...
	Try {
		...
		$Ngen = (Get-ChildItem C:\WINDOWS\Microsoft.NET\Framework64 -Filter "ngen.exe" -Recurse | Sort-Object LastWriteTime -Descending | Where-Object { $_.Length -gt 0 } | Select-Object -First 1).FullName
		"Found $Ngen"
